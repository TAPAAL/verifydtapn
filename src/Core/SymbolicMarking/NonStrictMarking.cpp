...
{
	boost::shared_ptr<TAPN::TimedArcPetriNet> NonStrictMarking::tapn;
	...
	void NonStrictMarking::AddTokens(const std::list<int>& placeIndices)
	{
	...
	void NonStrictMarking::RemoveTokens(const std::set<int>& tokenIndices)
	{
	...
	void NonStrictMarking::InitMapping()
	{
	...
	relation NonStrictMarking::ConvertToRelation(relation_t relation) const
	{
		switch(relation)
		{
	...
	bool NonStrictMarking::IsUpperPositionGreaterThanPivot(int upper, int pivotIndex) const
	{
	...
	void NonStrictMarking::Swap(int i, int j)
	{
		DiscreteMarking::Swap(i,j);
		dbm.swapClocks(mapping.GetMapping(i), mapping.GetMapping(j));
	}

	void NonStrictMarking::Print(std::ostream& out) const
	{
		out << "Placement: ";
