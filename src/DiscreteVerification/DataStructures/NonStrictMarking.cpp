...
NonStrictMarking::NonStrictMarking() : inTrace(false), children(0), passed(false), generatedBy(NULL){
	// TODO Auto-generated constructor stub

}

NonStrictMarking::NonStrictMarking(const TAPN::TimedArcPetriNet& tapn, const std::vector<int>& v) : inTrace(false), children(0), passed(false), generatedBy(NULL){
	int prevPlaceId = -1;
...
NonStrictMarking::NonStrictMarking(const NonStrictMarking& nsm) : inTrace(false), children(0), passed(false), generatedBy(NULL){
...
void NonStrictMarking::AddTokenInPlace(TAPN::TimedPlace& place, Token& token){
	for(PlaceList::iterator pit = places.begin(); pit != places.end(); pit++){
		if(pit->place->GetIndex() == place.GetIndex()){
