...
bool TimeDartReachabilitySearch::run(){
	...
	if(handleSuccessor(&initialMarking, NULL, INT_MAX)){
		return true;
	}
...
bool TimeDartReachabilitySearch::handleSuccessor(NonStrictMarkingBase* marking, WaitingDart* parent, int upper){
        int start = 0;
        if(options.getTrace() != VerificationOptions::NO_TRACE){
